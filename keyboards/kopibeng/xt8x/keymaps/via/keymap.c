...
bool BLINK_EFFECT_STATE = true;

bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    bool blink_state = BLINK_EFFECT_STATE;
    switch (keycode) {
        case SCRL_FX:
            if (record->event.pressed) {
                BLINK_EFFECT_STATE = !blink_state;
            }
            return true;
        default:
            if (BLINK_EFFECT_STATE) {
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	...
	// Fn1 Layer
	[1] = LAYOUT_all(
		KC_TRNS, RGB_TOG, RGB_MOD, RGB_HUD, RGB_HUI, RGB_SAD, RGB_SAI, RGB_VAD, RGB_VAI, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS,          KC_TRNS, SCRL_FX, KC_TRNS,
